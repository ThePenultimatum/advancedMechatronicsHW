...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            //ay = 2*((imuData[10] & 0x00FF)|((imuData[11] & 0x00FF)<<8)); // 2 * because of
            ...
            if (incrementerVar == 10000) {
                ...
                int mouseAX = ceil((ax * 1.0 / 4095) * 3);
                int mouseAY = ceil((ay * 1.0 / 4095) * 3);
                _CP0_SET_COUNT(0);
                while (_CP0_GET_COUNT() < 100000) {
                    Nop();
                }
                ...
                appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
                appData.xCoordinate = (int8_t) mouseAX;
                appData.yCoordinate = (int8_t) mouseAY;
                incrementerVar = 0;
            } else {
                appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
