...
void writeVertAxis(unsigned int startX, unsigned int startY) {
    unsigned int axisLength = AXISLENGTH;
    int i = 0;
    for (i=0; i < axisLength;i++) {
        LCD_drawPixel(startX, startY+i, ILI9341_RED);
    }
    ;
}

void writeHorizAxis(unsigned int startX, unsigned int startY, unsigned int numPoints) {
    unsigned int axisLength = AXISLENGTH;
    int i = 0;
    for (i=0; i < numPoints;i++) {
        LCD_drawPixel(startX+i, startY, ILI9341_RED);
    }
    ;
}

/*void plotPoint(unsigned int x, unsigned int y, unsigned int startX, unsigned int startY) {
    ;
}*/

void plotGraph(unsigned int startX, unsigned int startY, unsigned int numPoints, unsigned char *points) {
    writeVertAxis(startX, startY);
    writeHorizAxis(startX, startY+AXISLENGTH, numPoints);
    int i = 0;
    for (i = 0; i < numPoints; i++) {
        unsigned char p = points[i];
        unsigned int scaledVal = ceil((((float) p) / 240) * 50);
        LCD_drawPixel(startX+1+i, startY+AXISLENGTH-scaledVal, ILI9341_WHITE);
    }
}
